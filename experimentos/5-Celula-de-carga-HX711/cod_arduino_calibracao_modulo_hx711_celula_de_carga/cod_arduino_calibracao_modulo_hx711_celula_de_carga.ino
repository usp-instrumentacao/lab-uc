...
* 
* Exemplo:
*         UTILIZANDO CÉLULA DE CARGA ALIADA A MÓDULO AMPLIFICADOR HX711 (AJUSTE DE FATOR DE CALIBRAÇÃO)
* 
* Autores:
...
//Definição pino dos botões incremento e decremento do fator de calibração
#define INC_BTN_PIN 25
#define DEC_BTN_PIN 33
...
void setup() 
{
  ...
  pinMode(INC_BTN_PIN, INPUT);
  attachInterrupt(digitalPinToInterrupt(INC_BTN_PIN), interrup_function_one, RISING); 

  //Definição do pino do botão STOP e modo de interrupção
  pinMode(DEC_BTN_PIN, INPUT);
  attachInterrupt(digitalPinToInterrupt(DEC_BTN_PIN), interrup_function_two, RISING); 
...
void loop()
{
  ...
  noInterrupts(); //desabilita interrupção
      ...
      Serial.println("Fator de calibração  = " + String(fator_calibracao));
      Serial.println("Valor informado pelo módulo hx711 = " + String(medida_hx711) + " kg");
      Serial.println();
      
      //Exibir valor no display
      display_lcd.clear();
      display_lcd.setCursor(0,0);
      display_lcd.print("FC: " + String(fator_calibracao));
      display_lcd.setCursor(0,1);
      display_lcd.print("Massa: " + String(medida_hx711) + " kg");
...
// Funções auxiliares -------------------------------------------------------------------------------------
  ...
  {
     fator_calibracao = fator_calibracao + 50;//aumenta fator de calibracao
     delay(20);
  }

  //Função de interrupção 2
  void interrup_function_two() 
  {
     fator_calibracao = fator_calibracao - 50;//diminui fator de calibracao
     delay(20);
  }
